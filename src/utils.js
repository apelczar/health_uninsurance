...
export function lineChart(data) {
  //Create line charts for scrolling section
  ...
  const yMax = max(data, function(d) {
    return Number(d.percent_uninsured);
  });
  const y = scaleLinear()
    .domain([0, yMax + 0.02])
    .nice()
    .range([lineHeight, 0]);
  svgLine.append('g').call(axisLeft(y).ticks(5));
  ...
  const path = svgLine
    ...
    .attr('class', function(d) {
      return d[0].highlight1;
    });

  //y axis label
  svgLine
    .append('g')
    .attr('transform', `translate(-30, ${lineHeight / 2})`)
    .append('text')
    .text('Uninsured Rate (%)')
...
export function lineChart2(data) {
  //Create interactive line chart below scrolling section
  ...
  const y = scaleLinear()
    ...
    .nice()
    .range([lineHeight, 0]);
  svgLine2.append('g').call(axisLeft(y).ticks(5));
  ...
  function hover(svgLine2, path) {
    ...
    const dot = svgLine2.append('g').attr('display', 'none');
    ...
    function moved(event) {
      ...
      path
        .attr('stroke', d => (d === s ? '#08306B' : '#ddd'))
        .filter(d => d === s)
        .raise();
      dot.attr(
        'transform',
        `translate(${x(xAcc(s[i]))},${y(s[i].percent_uninsured)})`,
      );
...
export function barChart(data) {
  //create bar charts at bottom of page
  ...
  let useData = data.filter(
    x => x.Location === stateShow && x.category != 'Overall',
  );
  ...
  svg
    .append('g')
    .attr('transform', `translate(-30, ${barHeight / 2})`)
    .append('text')
    .text('Uninsured Rate (%)')
    .attr('transform', 'rotate(-90)')
    .attr('text-anchor', 'middle');
  ...
  const medicaidBox = select('.totalText').append('p');

  function renderChart() {
    const t = transition().duration(300);
    let dataTotals = data.filter(
      x => x.Location === stateShow && x.detail === 'Total',
    );
    let useData = data.filter(
      x => x.Location === stateShow && x.category != 'Overall',
    );
    let medicaidStatus = data.filter(
      x => x.Location === stateShow && x.detail === 'Medicaid',
    );
    ...
    totalBox.text(
      `In 2018, ${overallUninsured}% of ${stateShow} residents did not have health insurance.`,
    );
    ...
    if (stateShow === 'United States') {
      ...
      let expandIndicator =
        medicaidStatus[0].percent_uninsured > 0.5 ? 'had' : 'had not';
  ...
  container.append('g').each(function(d) {
...
export function legend({
  ...
  //This function was written by Mike Bostock: https://observablehq.com/@d3/color-legend

  const svg = select('#mapLegend')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
